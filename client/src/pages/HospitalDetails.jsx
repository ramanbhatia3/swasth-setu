  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8 border-b border-slate-200">
          <div className="col-span-1 md:col-span-2 space-y-8">
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {hospital.specializations.map((spec, i) => <span key={i} className="px-3 py-1.5 bg-slate-100 text-slate-800 text-sm font-medium rounded-lg">{spec}</span>)}
              </div>
            </section>
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {hospital.facilities.map((fac, i) => <span key={i} className="px-3 py-1.5 bg-emerald-50 text-emerald-800 text-sm font-medium border border-emerald-100 rounded-lg">{fac}</span>)}
              </div>
            </section>
          </div>
          ...
          <div className="col-span-1 space-y-6">
            ...
            <button 
              ...
              className="w-full py-3 bg-red-50 text-red-600 font-medium rounded-lg border border-red-100 hover:bg-red-100 transition-colors flex justify-center items-center gap-2"
            >
        ...
        <div className="p-8 bg-white">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => requireAuth(() => setIsReviewModalOpen(true))} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-blue-700 transition-colors text-sm shadow-sm flex items-center gap-2">
                ...
                <div key={review._id} className="p-5 border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      {review.user?.profileImage ? <img src={review.user.profileImage} alt="User" className="w-10 h-10 rounded-full object-cover border border-slate-200" /> : <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600"><User size={20} /></div>}
                      <div>
                        <p className="font-semibold text-slate-900 text-sm">{review.user?.name || "Anonymous User"}</p>
                        <p className="text-xs text-slate-500">{new Date(review.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 bg-yellow-50 text-yellow-700 px-2 py-1 rounded text-xs font-bold border border-yellow-100"><Star fill="currentColor" size={12} /> {review.rating}</div>
                  </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
              ...
              <form onSubmit={handleReviewSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
                {reviewError && <div className="p-3 bg-red-50 text-red-700 text-sm rounded-lg flex items-start gap-2"><AlertCircle size={16} className="mt-0.5 shrink-0" /> {reviewError}</div>}
                ...
                <div>
                  ...
                  <textarea required maxLength={1000} rows={4} value={reviewForm.experience} onChange={(e) => setReviewForm({...reviewForm, experience: e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 resize-none text-sm" />
                </div>
                <div className="pt-2 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsReviewModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
                  <button type="submit" disabled={submittingReview} className="px-5 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg transition-colors shadow-sm disabled:opacity-70">{submittingReview ? 'Submitting...' : 'Submit Review'}</button>
                </div>
              </form>
            </motion.div>
          </div>
      ...
      <AnimatePresence>
        {isReportModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border-t-4 border-red-600">
              <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50">
                <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2"><AlertTriangle className="text-red-600" /> Formal Hospital Report</h2>
                <button onClick={() => setIsReportModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
              </div>
              
              <form onSubmit={handleReportSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
                {reportError && <div className="p-3 bg-red-50 text-red-700 text-sm rounded-lg flex items-start gap-2"><AlertCircle size={16} className="mt-0.5 shrink-0" /> {reportError}</div>}
                {reportSuccess && <div className="p-3 bg-emerald-50 text-emerald-700 text-sm rounded-lg flex items-start gap-2"><CheckCircle size={16} className="mt-0.5 shrink-0" /> {reportSuccess}</div>}
                ...
                <div>
                  ...
                  <select value={reportForm.category} onChange={(e) => setReportForm({...reportForm, category: e.target.value})} className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 bg-white sm:text-sm">
                ...
                <div>
                  ...
                  <textarea required rows={4} value={reportForm.description} onChange={(e) => setReportForm({...reportForm, description: e.target.value})} placeholder="Describe the issue in detail..." className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 resize-none sm:text-sm" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 flex items-center gap-1"><UploadCloud size={16} /> Optional Evidence (Image/PDF)</label>
                  <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setReportFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 border border-slate-300 rounded-lg p-1" />
                  <p className="text-xs text-slate-500 mt-1">Max size: 5MB</p>
                </div>

                <div className="pt-2 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsReportModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
                  <button type="submit" disabled={submittingReport || reportSuccess} className="px-5 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors shadow-sm disabled:opacity-70">
